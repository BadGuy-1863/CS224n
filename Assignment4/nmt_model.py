...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded) #padding
        X = pack_padded_sequence(X, lengths=source_lengths) #packing

        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) #obtains values using LSTM layer, enc_hiddens is packed sequence
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings.target(target_padded) #padding

        for Y_t in torch.split(Y, 1):
            squ = torch.squeeze(Y_t, dim = 0)
            Ybar_t = torch.cat((squ, o_prev), dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = F.softmax(e_t, dim=1) #bxsrc_len
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens), dim=1) #got a_t
