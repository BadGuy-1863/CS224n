...
def negSamplingLossAndGradient(
    ...
    out = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss -= np.log(out)

    gradCenterVec += outsideVectors[outsideWordIdx]*(out-1.0)
    gradOutsideVecs[outsideWordIdx] = (out-1.0) * centerWordVec
    ### Please use your implementation of sigmoid in here.
    u_k = outsideVectors[negSampleWordIndices]
    neg = sigmoid(-np.dot(u_k, centerWordVec))
    loss += np.sum(-np.log(neg))
    gradCenterVec += np.dot(neg-1.0, u_k)*(-1)
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - neg[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        outIdx = word2Ind[word]
        l, cV, oV = word2vecLossAndGradient(
            currentCenterWordVec,
            outIdx,
            outsideVectors,
            dataset)
        loss += l
        gradCenterVecs[currentCenterWordIdx] += cV
        gradOutsideVectors += oV
